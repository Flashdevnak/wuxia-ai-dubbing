...
async function api(path,opts={}){if(IS_GITHUB_PAGES&&!window.WUXIA_API_BASE) throw new Error('GitHub Pages เป็นหน้าเว็บ Preview — ต้องเชื่อม backend/R2 ก่อนใช้งานจริง'); const r=await fetch(API_BASE+path,{headers:{'content-type':'application/json',...(opts.headers||{})},...opts}); const data=await r.json().catch(()=>({})); if(!r.ok)throw new Error(data.error||`HTTP ${r.status}`); return data;}
...
async function uploadFile(file){
  ...
  for(let i=0;i<total;i++){
    ...
    while(attempt<4&&!done){attempt++; try{const r=await fetch(`${API_BASE}/api/uploads/part?key=${encodeURIComponent(key)}&uploadId=${encodeURIComponent(uploadId)}&partNumber=${i+1}`,{method:'PUT',body:blob});const d=await r.json();if(!r.ok)throw new Error(d.error||'upload failed');parts.push({partNumber:d.partNumber,etag:d.etag});done=true;}catch(e){lastErr=e;await new Promise(res=>setTimeout(res,700*attempt));}}
...
function bind(){
  ...
  document.body.addEventListener('click',async e=>{const jb=e.target.closest('[data-delete-job]');if(jb&&confirm('ลบงานนี้และไฟล์ที่เกี่ยวข้องหรือไม่?')){await api('/api/jobs/'+jb.dataset.deleteJob,{method:'DELETE'});await Promise.all([loadJobs(),loadFiles(),loadStorage()]);}const fb=e.target.closest('[data-delete-file]');if(fb&&confirm('ลบไฟล์นี้เพื่อคืนพื้นที่หรือไม่?')){await api('/api/files?key='+encodeURIComponent(fb.dataset.deleteFile),{method:'DELETE'});await Promise.all([loadFiles(),loadStorage()]);}});
...
if(IS_GITHUB_PAGES&&!window.WUXIA_API_BASE){
  ...
  $('#message').textContent='หน้าเว็บออนไลน์แล้ว · โหมด Preview: ขั้นอัปโหลด/AI ต้องเชื่อม Storage + Backend';
  $('#topStorage').textContent='Preview';
  $('#homeJobs').textContent='หน้า Preview พร้อมใช้งาน · ยังไม่เชื่อมคิวประมวลผลจริง';
}else{
  const badge=$('#deployMode'); if(badge) badge.textContent='ระบบออนไลน์';
